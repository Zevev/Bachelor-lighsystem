...
process.umask = function() { return 0; };
      ...
      //Start of algortithm that controls the lights
      
    const axios = require("axios"); // Needed together with node to sendt http requests
    document.getElementById("lightOn").addEventListener("click", lightButtonOn);
    document.getElementById("lightOff").addEventListener("click", lightButtonOff);
    document.getElementById("lightAutomatic").addEventListener("click", lightButtonAutomatic);
    document.getElementById("runCalcBtn").addEventListener("click", calculateTimes);
    const cb = document.querySelector(`#lightOn`);
  
  
    const host = "10.0.0.2"; // IP-adress for the bridge
    ...
    let sleepMinute = 0;
    let startDate;
    let correctionMinute;
    
    
    const turnLightOnOrOff = async (lightId, on, bri, transitiontime, ct) => {
        const url = `http://${hueUrl}/lights/${lightId}/state`
    ...
    turnLightOnOrOff(1, true, 215, 540*10000, 5); //Turning lights on on startup
    turnLightOnOrOff(2, true, 215, 540*10000, 5);

    function calculateTimes(){ //Getting times calculated on the website
      wakeUpMinute = targetStartMinute;
      wakeUpHour = targetStartHour;
      sleepHour = targetStopHour;
      sleepMinute = targetStopMinute;
      correctionMinute = correctionTimeInMinutes;
    ...
    setInterval(function(){ // Set interval for checking
        ...
        console.log("targetstopbundle " + wakeUpMinute)
        if(startDate < date){ //Checking if a day has passed
          console.log("A day has passed");
          wakeUpMinute += correctionTime;
          startDate == date;
        } else {
          console.log("A day has not passed");
        }

        if(wakeUpMinute >= 60){ // Making sure the minutes cant go over 60
          wakeUpHour++;
          wakeUpMinute = wakeUpMinute-60;
        }

        if(date.getHours() === wakeUpHour && date.getMinutes() === abs(wakeUpMinute-45)){ // Check the time waking up
            turnLightOnOrOff(1, true, 153, 170*10000, 412);
            turnLightOnOrOff(2, true, 153, 170*10000, 412);
        }
    
        if(date.getHours() === wakeUpHour && date.getMinutes() === wakeUpMinute+30){ // Check the time for changing color temp midday
    ...
    if(cb.checked === true){ //Turning on or off the lights with one switch
      lightButtonOn()
    }
    ...
    function lightButtonOn() {
        clearInterval;
        turnLightOnOrOff(1, true, 254, 2, 10)
        turnLightOnOrOff(2, true, 254, 2, 10)
    }
